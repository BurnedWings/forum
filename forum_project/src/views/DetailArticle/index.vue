<template>
  <div class="article-view">
    <div class="article-view-container">
      <div class="article-container">
        ...
        <div class="author-info">
          <img v-if="authorInfo.image" @click="toUserView" class="author-image" :src="$myBaseUrl + authorInfo.image" />
          ...
          <div class="concern-btn">
            <div v-if="isConcern === null || isConcern === false" @click="concernOneUser" class="concern-button">关注
            </div>
        ...
        <div class="share-box">
          <ul class="box-container">
            ...
            <li @click="shareToWeibo" class="share-box-item">
              <i class="iconfont icon-weibo"></i>
            </li>
            <li class="share-box-item">
              <i class="iconfont icon-weixin"></i>
              <div class="shareBox">
                <vue-qr :logoSrc="imageUrl" :text="articleUrl" :size="170"></vue-qr>
              </div>
            </li>
            <li @click="shareToQQkj" class="share-box-item">
              <i class="iconfont icon-qqkongjian"></i>
            </li>
            <li @click="shareToQQ" class="share-box-item">
        ...
        <div class="comments-body">
          <div v-for="(comment, myIndex) in commentsList" :key="comment._id" class="comment-item">
            ...
            <div class="user-avatar">
              <img @click="toDetailUserView(comment.user._id)" v-if="authorInfo.image"
                :src="$myBaseUrl + comment.user.image" alt />
            </div>
            <div class="comment-content">
              ...
              <div class="comment-bottom">
                ...
                <div @click="kudosTheComment(comment._id, comment.user._id)" class="kudos">
                  <i v-if="commentStatusArr === null || commentStatusArr[myIndex].status === 0"
                    class="iconfont icon-dianzan"></i>
                  <i v-if="commentStatusArr != null && commentStatusArr[myIndex].status === 1"
                    class="my-dianzan-icon iconfont icon-dianzan"></i>&nbsp;
                ...
                <div class="comment-edit">
                  <el-popconfirm v-if="userId === comment.user._id" title="你确定要删除评论嘛?"
              ...
              <div :id="'reply-container' + myIndex" class="reply-container">
                <div class="reply-item" v-for="(reply, index) in comment.replyList" :key="reply._id">
                  <div class="reply-left">
                    <img @click="toDetailUserView(reply.user._id)" v-if="authorInfo.image"
                      :src="$myBaseUrl + reply.user.image" alt />
                  </div>
                  <div class="reply-right">
                    ...
                    <div class="reply-item-bottom">
                      ...
                      <div @click="kudosTheReply(reply._id, reply.user._id)" class="kudos">
                        <i v-if="commentStatusArr === null || commentStatusArr[myIndex].reply[index] === 0"
                          class="iconfont icon-dianzan"></i>
                        <i v-if="commentStatusArr != null && commentStatusArr[myIndex].reply[index] === 1"
                          class="my-dianzan-icon iconfont icon-dianzan"></i>&nbsp;
                        <span>{{ reply.favoritesCount > 0 ? reply.favoritesCount : '' }}</span>
                      </div>
                      <div @click="showReplyInput(myIndex, reply.user, reply.user._id, comment._id, reply._id)"
                        class="reply">回复</div>
                      <div class="reply-edit">
                        <el-popconfirm title="你确定要删除评论嘛?" class="edit-container" @confirm="deleteOwnReply(reply._id)">
                          <i slot="reference" v-if="userId === reply.user._id" class="my-edit-icon el-icon-delete"></i>
                        </el-popconfirm>
                        <i v-if="userId != reply.user._id" @click="toEditReply(myIndex, index)"
                          class="my-edit-icon el-icon-more"></i>
                        <div @mouseleave="closeReplyBox(myIndex, index)" :ref="'replyEditBox' + myIndex + index"
                          class="to-edit-reply">
              ...
              <div :id="'two-more-message' + myIndex" @click="showMoreReply(myIndex)"
                v-if="comment.replyList.length > 2" class="two-more-message">共{{ comment.replyList.length }}条,点击查看</div>
        ...
        <div class="side-toolbar-directory">
          <div class="directory">
            <div class="directory-body">
              <div v-if="collectionInfo._id" class="title">
                目录
                <span class="title-message">来自合集：{{ collectionInfo?collectionInfo.title: ''}}&nbsp;></span>
              </div>
              <div style="text-align: center;" v-else class="title">该文章没有加入合集</div>
              <div class="directory-body-container">
                <div v-for="(article, index) in collectionInfo.articleList " :key="article._id" class="data-item">
                  <div @click="toDetailArticle(article._id)" ref="directoryItem" class="data-title">
    ...
    <el-dialog class="my-dialog" title="加入收藏夹" :visible.sync="dialogTableVisible">
      <div class="line"></div>
      <div class="inner-collection-container">
        <div v-for="(collection, index) in collectionList" :key="collection._id" class="collection-item">
          <div class="data-title">
            <div class="side-bar"></div>
            <i class="myIcon el-icon-wallet"></i>
            <div class="title-content">{{ collection.collectionName }}</div>
            <span @click="collectionArticle(collection._id)" class="collection-status">{{
              isCollectionArr[index]=== 1 ?
                '已收藏' : '收藏'
            }}</span>
          </div>
        </div>
    ...
    <el-dialog title="请编辑举报理由" :visible.sync="dialogVisible" width="30%" :before-close="handleClose"
      close="my-return-box" :modal-append-to-body="false">
      ...
      <el-input style="width:95%;margin-top:20px;margin-bottom:20px;" type="textarea" :rows="6" resize="none"
        maxlength="160" show-word-limit placeholder="请输入具体描述" v-model="textarea"></el-input>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false">取 消</el-button>
        <el-button v-if="targetReportComment" style="margin-right:21px;" type="danger"
          @click="reportTheComment">提交</el-button>
        <el-button v-else-if="targetReportReply" style="margin-right:21px;" type="danger"
          @click="reportTheReply">提交</el-button>
...
const CommentInput = () => import("@/components/CommentInput");
import ReplyInput from '@/components/ReplyInput'
import router from "@/router";
import { toQQzone, toQQ, toWeibo } from "@/utils/share";
...
export default {
  ...
  data() {
    return {
      imageUrl: require("./images/default.jpg"),
      detailArticle: {},
      authorInfo: {},
      commentsList: null,
      chooseBox: null,
  ...
  computed: {
    ...
    articleUrl() {
      return `http://192.168.43.105:8080/detailArticle/${this.$route.params.articleId}`;
    }
  },
  watch: {
    $route(to, from) {
  ...
  methods: {
    toDetailArticle(articleId) {
      this.refStyle();
      // this.$refs.directoryItem[this.activeIndex].style.backgroundColor =
      //   "var(--theme_search_data_title_hover_bg)";
      // this.$refs.directoryItem[this.activeIndex].style.color =
      //   "var(--theme_search_input_blue_color)";
      this.$router.push({
        name: "detailArticle",
        params: {
          articleId
    ...
    refStyle() {
      this.$refs.directoryItem[this.activeIndex].style.backgroundColor =
        "var(--theme_inner_bg_color)";
      this.$refs.directoryItem[this.activeIndex].style.color =
        "var(--theme_font_color)";
    },
    intoDirectory() {
      if (this.collectionInfo._id) {
        this.$nextTick(() => {
          this.$refs.directoryItem[this.activeIndex].style.backgroundColor =
            "var(--theme_search_data_title_hover_bg)";
          this.$refs.directoryItem[this.activeIndex].style.color =
    ...
    //分享到qq
    shareToQQ() {
      const articleId = this.detailArticle._id;
      const title = this.detailArticle.title;
      const desc = this.detailArticle.description;
      toQQ(
        `http://192.168.43.105:8080/detailArticle/${articleId}`,
        `${title}`,
        `${desc}`
      );
    },
    ...
    shareToWeibo() {
      ...
      const title = this.detailArticle.title;
      const desc = this.detailArticle.description;
      toWeibo(`http://127.0.0.1:8080/detailArticle/${articleId}`, `${title}`);
    },
    //查看更多评论
    showMoreReply(index) {
    ...
    async concernOneUser() {
      if (!this.userId) return this.$router.push("/login");
      const fans = {};
    ...
    async getArticleOfCollection() {
      // this.activeIndex = null
      // this.collectionInfo = {}
      const ret = await this.$API.user.getArticleOfCollection(
        this.$route.params.articleId
      );
      if (ret.code === 200) {
        if (ret.collection) {
          this.collectionInfo = ret.collection;
          ret.collection.articleList.forEach((item, index) => {
            if (item._id === this.$route.params.articleId) {
